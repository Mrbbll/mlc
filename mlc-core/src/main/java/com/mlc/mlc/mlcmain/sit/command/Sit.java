package com.mlc.mlc.mlcmain.sit.command;
...
public class Sit implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String @NotNull [] strings) {
        ...
        EntityScheduler entityScheduler = itemDisplay.getScheduler();
        entityScheduler.runAtFixedRate(instance,task->{
            Block block1 = itemDisplay.getLocation().clone().add(0,-0.1,0).getBlock();
            if(!block1.isSolid()){
                itemDisplay.remove();
                task.cancel();
            }
